...
- 拔插串口、点击打开/关闭串口，更新对应的UI显示以及提示信息
...
- 条件如果返回True，说明现在仍在等待继续扫码，不会执行后续流程
...
3. PL2303 (Prolific)
    ...
    PID：24577
    备注：老设备常见，新版 Win10/11 驱动麻烦

4. FT232 (FTDI，工业级最贵)

    PID：24576
    备注：稳定性极强，高端设备、医疗设备常用
...
[ 0x52, 0x0D, 0x0A ]
```
- python中同一个ASCII码有两种表现形式
- 不加 `b`：**str 字符串** → 给人看的文本
- 加 `b`：**bytes 字节** → 给硬件 / 串口 / 网络发的原始二进制
- 硬件通信必须用 **带 b 的 bytes**，因为它直接就是 ASCII 字节流；普通字符串要先 encode 转成 bytes 才能发。
...
# 匹配
match = WEIGHT_LINE_PATTERN.search(line)

if match:
    # 提取分组里的数字 ← 就是 ([\d.]+) 抓到的内容
    weight = match.group(1)
    print(weight)  # 输出 2.17
```
...
- 流程刚开始，会调用这一个函数收集储存SN，`save_sn_to_list`调用一次发现`sn_save_list`没有填充完毕，才会将`sn_up_enable`置为`true`，其余状态都是`false`
...
- 上述这个函数，传入SN以及工站相关数据，打包后发送给MES系统，MES会根据上报的内容做出一定的反馈

```python
API=HN_CheckLotSN
UserParameter=AKJCTGNCS
UserData={
    "LotSN": "SN20260001",
    "TestDevice": "治具-A01"
}
...
- 经过测试，如果传入的`user_parameter`（也就是dev）是一个MES没有定义的值，不会进行任何报错，返回值也是`True`
...
- 数据入队，组装打印日志作为提示
- 至于如何将数据从串口发送给体重计，大概是串口线程不断发送“发送队列”中的数据；还有一个“接收队列”数据不断储存，等待后续随时被调用（这就是单开一个线程的作用）
...
- 上报函数需要和`check_sn_is_ok`对比阅读，都是需要向MES系统传输数据

```python
{
  "API": "HN_SubmitTestData",
  "UserParameter": "HNWEIGH106_RP_TODO",
  "UserData": {
    "LotSN": "ABC123456",
    "TestDevice": "WEIGHT_STATION_01",
    "UserCode": "001", 
    "TestResult": "OK",
    "StartTime": "2024-01-15 10:30:25",
    "EndTime": "2024-01-15 10:30:28",
    "TestDataItem": [
      {
        "ItemName": "Weight",
        "ItemValue": "125.5",
...
1. 在 `sys.modules` 里登记 `test_tool.test`（先占位，表示「正在加载」）
2. 从第 1 行执行到文件最后一行（只执行顶层代码）
3. 执行完后，把完整的模块对象放进 `sys.modules`
4. 以后再次 import：不再执行 `test.py` 主体，直接返回缓存

因此：

- 会执行：`import`、赋值、`class`/`def` 的定义、`LoadCfg()` 这类调用
- 不会执行：函数体、`load_config()`、`test_run_process()` 等（除非在顶层被调用）

### celink_mes主体格式

1. 在`test.py`中建立了一个`LoadCfg`类，并使用`load_cfg = LoadCfg()`完成类的构造；
2. 之后在`test.load_config()`调用模块的加载函数，将`config.yaml`文件中的各种配置加载到`LoadCfg`类的成员变量之中，之后就可以直接调用类的成员变量，相当于读取`config.yaml`文件的成员变量的值；

**扫描完毕SN码值之后，需要将SN发送给MES系统进行检验：**

- 调用`check_sn_is_ok(sn="")`
...
- `load_cfg.dev = config['device_type']`；
- 其中，在`config.yaml`文件中`device_type`指工站编号；
- 在MES系统中`user_parameter`叫做用户码；MES系统根据UserParameter参数调用不同的业务逻辑

- `load_cfg.test_tool = config.get('test_tool', "治具未编码") `
-  测试工具编码，暂不使用（海能mes）

- `API`在检验SN时，使用`HN_CheckLotSN`；上传报告时，使用`HN_SubmitTestData`，只有这两种
- 接口在上传数据之前应先请求是否允许测试接口（ 校验，当接口返回OK时，再请求数据上传接口（ HN_CheckLotSN ）进行MES业务 HN_SubmitTestData ）上传测试数据

**数据上传参考格式：**
```json
{
  "API": "HN_CheckLotSN",
  "UserParameter": "HNXJCTQGNCS",
  "UserData": {
    "LotSN": "RV30TEST0001",
    "TestDevice": "治具未编码"
  }
...
**MES系统接口返回值：**
- 返回的消息结构是JSON字符串，仅仅包含两部分
- `ResultCode`:"OK"代表成功，"NG"代表失败
- `ResultMessage`：接口返回一句话，供ui打印作为提示
...
- `UserCode`代表工号，不是必填项目
- `result`是函数调用时传入的，`send_res = mes_run.send_report(test.test_start_time, end_time, sn, "OK")`，根据实际的测试结果是否在阈值内，由上位机判断，传结果给MES


**在实际上传到MES之前，还有一个参数列表需要预处理一下，后续一并上传到MES：**
...
def celink_mes_get_result_item(name="", value="", description="", result=False):
    ...
    return ret_dirt
```
- `celink_report = []`是一个列表，通过`add_report`传参，`celink_mes_get_result_item`将`add_report`传入的参数转化为MES接收约定的格式；
- 将结果加载到`celink_rep`列表，作为全局变量后续加载到`celink_send_report`的`"TestDataItem": mes_run.celink_report`参数，从而上传到MES

```json
{
    "LotSN": "AE6A060F32400001",
    "TestDevice": "治具未编码",
    "UserCode": "001",
    "TestResult": "NG",
    "StartTime": "2026-05-17 14:30:00",
    "EndTime": "2026-05-17 14:30:12",
    "TestDataItem": [
        {
            "ItemName": "打高压测试",
            "ItemValue": "3.640kV,AC,8.50mA,1",
            "ItemResult": "OK",
            "Description": "max:5mA min:0.1mA"
        }
    ]
}
```
...
`test`线程循环调用`test_run_process`函数：
...
`check_ser_connect_and_up_ui()`每次循环之前先检查串口连接情况，并更新ui显示；

当治具发送`开始测试`的指令后，`check_sn_enable`会置为`true`，如果之后开始扫码使得`(barcode_q.empty() is not True)`，那么`dev < 100`时`test`循环调用的`barcode_check_process`函数得以执行；
...
```python
	...
	return
```
- 字符串是按照十进制解析，所以int("050") = 50
- `_txd = rv30_proto_tx_dev_byte()`当进入`dev = 50`的分支时，根据设定返回一个数值作为`ser_send_data(dev=_txd, cmd=0x58, data=str_list)`，按理说应该返回十进制的`50`，但是方雄规定的似乎是`0x50`，所以在这个函数动态调整

- 因为解码结果是治具发送`开始测试`并且`读取扫码枪码值后`的第一步，如果失败发送两种失败指令；
- 第一次是反馈失败的sn码值，第二次是整个项目由上位机检测结果异常反馈失败的统一指令


- 码值验证成功，回复`ser_send_data(dev=_txd, cmd=0x57, data=str_list)`，在`test_serial_rx_data_handle`等待后续指令
